...
import alluxio.client.file.cache.remote.net.service.UpdateResponse;
...
public class MemoryAllocator {
  ...
  public List<ByteBuf> allocate(int size) {
    ...
    Preconditions.checkArgument(n == size);
    return l;
  }
  ...
  public List<ByteBuf> allocate0(int size, boolean isNew) {
    ...
    if(left > 0 && !isNew) {
      ...
      res0.add(ByteBufAllocator.DEFAULT.heapBuffer(left));
    }
    return res0;
  }
  ...
  public class PageAllocator {
    ...
    public void init() {
      for (int i = 0; i < mPageNum; i++) {
        mPages.add(ByteBufAllocator.DEFAULT.heapBuffer(mPageSize));
      }
    }
