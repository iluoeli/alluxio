...
public class ClientCacheContext implements CacheContext {
  ...
  public static String mTestFunctionName ;
  public static HashSet<Integer> mReadLockList = new HashSet<>();
  public static HashSet<Integer> mREadUnlockList = new HashSet<>();
  public static HashSet<Integer> mReadLockList1 = new HashSet<>();
  public static HashSet<Integer> mREadUnlockList1 = new HashSet<>();
  ...
  public static Set<LongPair> testSet = new HashSet<>();
    ...
    while (x != null) {
      if (begin >= x.getBegin() && end <= x.getEnd()) {
        ...
        if (unit.getEnd() > x.getEnd()) {
          unit = handleLeftCoincidence(x, unit, true, index, task);
          change = true;
        }
        if (unit.getBegin() < x.getBegin()) {
          if (change) unit.mCacheConsumer.removeFirst();
          unit = handleRightCoincidence(unit, x, true, index, task);
        }
        return unit;
      }
    }
    ...
    while (iter.hasPrevious()) {
      ...
      if (end <= right) {
        ...
        if (end < left) {
          ...
          if (end != left) return handleRightCoincidence(newUnit, current, true,
            bucketIndex, task);
        }
      } else {
  ...
  public CacheUnit getKey(long begin, long end, long fileId, Iterator iter, int bucketIndex,
    ...
    while (iter.hasNext()) {
      ...
      if (begin >= left) {
        if (end <= right) {
          newUnit = null;
          task.readLock(current.mBucketIndex);
          return current;
        }
        if (begin > right) {
   ...
   */
  public TempCacheUnit handleRightCoincidence(TempCacheUnit result, CacheInternalUnit current,
                                              boolean addCache, int bucketIndex, LockTask task) {
    if(needWriteLock()) {
      task.readUnlock(bucketIndex);
      task.writeLock(bucketIndex);
    }
  ...
  public TempCacheUnit handleLeftCoincidence(CacheInternalUnit current, TempCacheUnit result,
                                             boolean addCache, int bucketIndex, LockTask task)
  {
    if (needWriteLock()) {
      task.readUnlock(bucketIndex);
  ...
  public class RWLockManager implements LockManager {
    ...
    public void readUnlockWithRetry(long fileId, int bucketIndex) {
      ...
      if (Thread.currentThread().getId() == 1) {
        mREadUnlockList1.add(bucketIndex);
      } else {
        mREadUnlockList.add(bucketIndex);
      }
    }
