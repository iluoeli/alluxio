...
public class TempCacheUnit extends LinkNode<TempCacheUnit> implements CacheUnit {
  ...
  public int cache(long pos, int len, LinkedFileBucket bucket) throws IOException {
    ...
    if (hasResource()) {
      ...
      while (pos < end) {
        ...
        if (current != null && pos >= current.getBegin()) {
          ...
          if ((needDeleteFirst && pos > current.getBegin()) || (needDeleteLast && pos + current.getSize() == end)) {
            mCacheConsumer.poll();
            Queue<LongPair> q = new LinkedList<>();
            q.add(new LongPair(pos, Math.min(current.getEnd(), end)));
            List<CacheInternalUnit> first = current.split(q, bucket);
